...
import { LOGIN_QUERY } from '../graphql/queries'; // Import your mutation
...
function Signin() {
  ...
  useEffect(() => {
    if (data) {
      const response = data.glAdmin.GlAdmin;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <div className="absolute inset-y-0 right-0 pr-3 flex items-center cursor-pointer" onClick={togglePasswordVisibility}>
                {showPassword ? <EyeSlashIcon className="h-5 w-5 text-gray-500" aria-hidden="true" /> : <EyeIcon className="h-5 w-5 text-gray-500" aria-hidden="true" />}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm">
              <a href="#" className="font-medium text-[#00DDEB]">Forgot password?</a>
            </div>
          </div>

          <button type="submit" className="w-full bg-[#9854FF] text-white py-2 px-4 rounded-2xl">
